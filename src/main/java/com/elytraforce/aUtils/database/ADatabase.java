...
public class ADatabase extends AbstractDatabase<JavaPlugin>{
    public static ADatabase newDatabase() {
        return new ADatabase();
    }

    private ADatabase() {}

    public ADatabase open() {
        if (using == null)
            throw new IllegalStateException("Cannot open the Database without a plugin using! Did you call Database#withPluginUsing?");
        logger = AUtil.getUtils().getLogger();
    ...
    public void queryAsync(String sql, Object[] toSet, Callback<ResultSet> callback) {
        asyncQueue.execute(() -> {
            try (Connection connection = source.getConnection()) {
                AUtil.getUtils().debug("(Query) Successfully got a new connection from hikari: " + connection.toString() + ", catalog: " + connection.getCatalog());
                try (PreparedStatement statement = connection.prepareStatement(sql)) {
                    AUtil.getUtils().debug("(Query) Successfully created a PreparedStatement with the following: " + sql);
                    if (toSet != null) {
                        for (int i = 0; i < toSet.length; i++) {
                            statement.setObject(i + 1, toSet[i]);
                        }
                    }
                    AUtil.getUtils().debug("(Query) Successfully set objects. Executing the following: " + statement.toString().substring(statement.toString().indexOf('-') + 1));
