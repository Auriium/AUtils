...
public class AParticle {
     ...
     */
    public AParticle(Particle particle, Callable<Location> locationCaller, Location location, int count,
    ...
    public AParticle(Particle particle, Location location, int count, double offsetx, double offsety, double offsetz) {
        this(particle, null, location, count, offsetx, offsety, offsetz, 0);
    }

    public AParticle(Particle particle, Location location, int count) {
        this(particle, location, count, 0, 0, 0);
    }

    public AParticle(Particle particle, Location location) {
    ...
    public static AParticle colored(Location location, int r, int g, int b, float size) {
        AParticle dust = new AParticle(Particle.REDSTONE, null, location, 1, 0, 0, 0, 0);
    ...
    public static AParticle colored(Location location, Color color, float size) {
    ...
    public static AParticle simple(Location location, Particle particle) {
        Objects.requireNonNull(particle, "Cannot build ParticleDisplay with null particle");
        return new AParticle(particle, null, location, 1, 0, 0, 0, 0);
    }
    ...
    public static AParticle display(Location location, Particle particle) {
        Objects.requireNonNull(location, "Cannot display particle in null location");
        AParticle display = simple(location, particle);
        display.spawn();
        return display;
    }
    ...
    public static AParticle fromConfig(Location location, ConfigurationSection config) {
        Objects.requireNonNull(config, "Cannot parse ParticleDisplay from a null config section");
        Particle particle = AParticles.getParticle(config.getString("particle"));
        ...
        AParticle display = new AParticle(particle, null, location, count, offsetx, offsety, offsetz, extra);
    ...
    public static Location rotate( Location location, double x, double y, double z,  Vector rotation) {
        ...
        Vector rotate = new Vector(x, y, z);
        AParticles.rotateAround(rotate, rotation.getX(), rotation.getY(), rotation.getZ());
    ...
    public AParticle withCount(int count) {
        this.count = count;
        return this;
    }
    ...
    public AParticle withExtra(double extra) {
        this.extra = extra;
        return this;
    }
    ...
    public AParticle withColor(Color color, float size) {
    ...
    public AParticle withEntity(Entity entity) {
    ...
    public AParticle withLocationCaller(Callable<Location> locationCaller) {
        this.locationCaller = locationCaller;
        return this;
    }
    ...
    public AParticle faceEntity(Entity entity) {
    ...
    public AParticle cloneWithLocation(double x, double y, double z) {
        AParticle display = clone();
    ...
    public AParticle clone() {
        AParticle display = new AParticle(particle, locationCaller, (location == null ? null : cloneLocation(location)), count, offsetx, offsety, offsetz, extra);
    ...
    public AParticle rotate(Vector vector) {
     ...
     * Rotations are only supported for some shapes in {@link AParticles}.
    ...
    public AParticle rotate(double x, double y, double z) {
    ...
    public AParticle offset(double x, double y, double z) {
        offsetx = x;
        offsety = y;
        offsetz = z;
        return this;
